...
async function driveHome(page, evidenceDir, basePath) {
  ...
  await page.getByRole('img', { name: /Sandra Camilo/i }).first().waitFor({ state: 'visible' });
  await page.getByText(/Stories with soul|Histórias com alma/i).first().waitFor({ state: 'visible' });
...
async function drivePlannedProducts(page, evidenceDir, basePath) {
  ...
  await dataTest(page, 'guideProductTitle1').filter({ hasText: 'Sintra Guide' }).waitFor({ state: 'visible' });
  await dataTest(page, 'guideProductStatus1').filter({ hasText: 'Coming soon' }).waitFor({ state: 'visible' });
  await dataTest(page, 'guideProductStatus2').filter({ hasText: 'Coming soon' }).waitFor({ state: 'visible' });
